...
# Configuration
MODEL_ID = "models/gemini-2.5-pro-preview-03-25"  # Adjust based on Google AI Platform's available models
SLIDES_PER_TOPIC = 5
TOTAL_SLIDES = 40
STATE_FILE = "presentation_state_gemini.json"
OUTPUT_FILE = "presentation_gemini.json"
...
# Create FAISS index with Sentence Transformers
def create_faiss_index(tagged_chunks):
    """Create a FAISS index for document chunks using Sentence Transformer embeddings."""
    try:
        ...
        return index, tagged_chunks, model
    except Exception as e:
        print(f"⚠️ Error creating FAISS index: {e}")
        raise

# Retrieve relevant chunks
def retrieve_chunks(index, tagged_chunks, model, query, section, k=20, used_chunks=None):
    """Retrieve relevant document chunks using Sentence Transformer embeddings and FAISS."""
...
def generate_slide_content(prompt, context, previous_slides, section, slide_number, client, section_summaries):
    """Generate a single slide using the Gemini API, ensuring content aligns with ALTAI."""
        ...
        try:
            ...
            if attempt == max_retries - 1:
                ...
                if suggested_title == "Accuracy" and section == "Technical Robustness and Safety":
                    fallback_content = "ALTAI requires assessing AI accuracy through task-specific metrics, ensuring reliable predictions for unseen data. Did you evaluate accuracy metrics for your AI system?"
                    return {"title": suggested_title, "content": fallback_content}, section_summaries
...
def main():
    """Generate a 40-slide JSON presentation from the ALTAI PDF using Gemini API and Sentence Transformers."""
    ...
    # Initialize Gemini client
    print("⚠️ Initializing Gemini client...")
    try:
        client = initialize_gemini_client()
        if not test_gemini_client(client):
            print("❌ Gemini API connection failed. Exiting.")
            return
    except Exception as e:
    ...
    index, tagged_chunks, model = create_faiss_index(tagged_chunks)
    ...
    for batch in range(state["current_batch"], total_batches):
        ...
        for i in range(SLIDES_PER_TOPIC):
            ...
            subtopics = {
                ...
                "Societal and Environmental Well-being": ["environment", "work", "society", "consultation", "democratic effects"],
            ...
            context_chunks, used_chunks = retrieve_chunks(index, tagged_chunks, model, query, section, k=20, used_chunks=used_chunks)
